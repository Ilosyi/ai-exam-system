  ...
  const pagination = useMemo(
    () => ({
      ...
      onChange: (page: number, pageSize?: number) =>
        setFilters((prev: QuestionFilters) => ({ ...prev, page, pageSize })),
    }),
    [filters.page, filters.pageSize, total],
  );

  const onCreate = useCallback(async (payload: Partial<Question>) => {
    // 创建题目后重新加载列表以显示最新记录
    await createQuestion(payload);
    await load();
  }, [load]);

  const onUpdate = useCallback(async (id: number, payload: Partial<Question>) => {

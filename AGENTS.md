...
如果 `AGENTS.md` 与旧实现、旧计划冲突，应以“当前代码真实状态 + 本文档约束”为准。

---

## 2. 项目定位

项目是一个面向教学场景的 AI 题库与考试系统，包含教师后台与学生考试端两套使用路径，并提供课程文档阅读与管理能力。
...
- `admin`
  可访问后台全部内容，包括用户管理和课程文档管理
- `teacher`
  可访问题库、试卷、班级、AI 出题、课程文档管理等教师端功能
- `student`
  仅访问首页、课程文档阅读与考试端
...
- 课程文档根目录固定为项目根目录下的 `course-docs/`
- 每门课程以独立目录存储，课程元数据文件为 `course.json`
- Markdown 正文以课程目录下的 `*.md` 文件存储
- `admin` 和 `teacher` 可以管理全部课程与文档
- 登录用户可以查看全部课程与文档
- 当前不支持图片或附件上传
...
- `user_classes`
  学生-班级多对多关系
- 课程文档
  file-backed 文件型数据，存储在 `course-docs/`，不进入 SQLite
...
- 课程文档读接口登录即可访问，写接口只允许 `admin` 和 `teacher`
